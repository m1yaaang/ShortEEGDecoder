'python EEGPT/finetune_EEGPT_combine_LoRA_conv_within_util.py'
...
if __name__ == "__main__":
    ...
    print(f"\n" + "="*40)
    print(f"[:] Within-Subject Training (subject: {subject_id})")
    ...
    for patch_idx in range(12,15):
    # for patch_idx in range(6,7):

        target_cls = 'All'
        ##FIXME - CONFIG(project name)
        # ------------------config---------------------------
        experiment_name = f"LORA_interp_500Hz_{subject_id}_P{patch_idx}_C{target_cls}_t{config['time_bin']}"
        exp_id = f"{train_date}_{experiment_name}"
        print(f"[*] Experiment ID: {exp_id}")
        print(f"[*] Mode: {'DEBUG (Overfit 1 batch)' if IS_DEBUG else 'TRAINING'}")

        base_dir = f"./EEGPT/within/T2_{config['sampling_rate']}Hz_t{config['time_bin']}/{exp_id}/"
