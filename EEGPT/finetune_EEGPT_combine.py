...
class LitEEGPTCausal(pl.LightningModule):       # !) Transformer(encoder -> decoder) structure -> classifier(encoder -> Linear(Flatten))
    ...
    def __init__(self, 
        ...
        self.fixed_train_path_idx = fixed_train_patch_idx
    ...
    def forward(self, x, mask = None):
        ...
        if self.training and (self.fixed_train_path_idx is not None):
            B, N, _ = z.shape
            time_mask = torch.zeros((B, self.model_input_len)).to(z.device)
            t_start = (self.fixed_train_path_idx)*self.time_bin
            t_end = (self.fixed_train_path_idx+1)*self.time_bin
    ...
    def training_step(self, batch, batch_idx):
        ...
        if batch_idx ==0:
            ...
            # self.log('Grad_Head', head_grad.abs().mean().item(), on_epoch=True, on_step=False, sync_dist=True)
            ...
            # self.log('Grad_Encoder', enc_grad.abs().mean().item(), on_epoch=True, on_step=False, sync_dist=True)
            # print("\n"+"="*30)
...
if __name__ == "__main__":
    ...
    # for fold in range(1):
        ...
        for train_patch_idx in train_patch_idx:
            ...
            # for target_cls in [target_cls]:
                ...
                # ------------------config---------------------------
                experiment_name = f"FC_MultiC_Timebin{time_bin}_F{current_fold}_P{train_patch_idx}_C{target_cls}"
                ...
                wandb_logger = WandbLogger(
                                    project="eegpt_combine3", 
                                    name=experiment_name,
                                    group=f"{exp_id}_Fold{current_fold}",
                                    id=f"{exp_id}_F{current_fold}_P{train_patch_idx}_C{target_cls}",
                                    tags=[f"Fold{experiment_name}",f"Patch{train_patch_idx}",f"Class{target_cls}"],
                                    save_dir=base_dir
                                    )
                ...
                if not IS_DEBUG: # INFERENCE
                    ...
                    print(f"OVR_Class{target_cls}_Patch{train_patch_idx}_Timebin{time_bin} Fold {fold+1} Best Validation Loss:{best_score}")
